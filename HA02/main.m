...
figure('Color','white','Position',[591  304  560  420]);
hold on, grid on;
...
for i=1:numel(idx)
    ...
    ylim([0,max(y)*1.1])
    title(['posterior distribution: ' name]), ylabel(name), xlabel 'state x', grid on;
...
hold on, grid on;
autocorr(v)
xlabel 'Lag', ylabel 'Sample autocorrelation', title 'Sample utocorrelation function of innovation'
...
% plot position
figure('Color','white','Position',[385  444  437  294]);
hold on, grid on;
...
% plot velocity
figure('Color','white','Position',[832  448  437  294]);
hold on, grid on;
...
xlabel('k - time step');
ylabel('position');
legend([p2 p3 p4 p5],'Location','southeast');
% fp.savefig('q2b-vel')
...
for i=1:numel(Qi)    
    ...
    legend([p1 p2 p3 p4 p5],'Location','southeast');
    % fp.savefig(sprintf('q2c-pos-Qi-%d',Qi(i))
end
