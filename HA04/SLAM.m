classdef SLAM < handle
    ...
    methods(Static)
        ...
        function pos0 = genValidRandParticles(N)
            % handle to generate new points within bounding box
            gen = @(N) [1;1] + diag([10 8])*rand(2,N);
            pos0 = gen(N);
            idx = zeros(N,1);
            while ~all(idx)
                idx = SLAM.isOnRoad(pos0(1,:),pos0(2,:));
                pos0 = [ pos0(:,find(idx)) gen(N-sum(idx)) ];
            end
        end
    ...
    methods
        ...
        function plotTruePos(obj)
            plot([obj.pos(1,:)+obj.pos(2,:)*1i],'-*')
        end
        
        function obj = loadPosition(obj,filename)
            track = load(filename);
            obj.pos = track.Xk;
        end
        
        function obj = drawPath(obj, filename)
